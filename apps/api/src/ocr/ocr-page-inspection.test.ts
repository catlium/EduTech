...
import { derivePageDetails, aggregatePagesText } from './ocr-coordinator.util.ts';
...
const CORRECTIONS = new Map([
  [
    2,
    { correctedText: 'FIXED PAGE 2', correctedBy: 'user-1', correctedAt: '2026-09-15T00:00:00Z' },
  ],
]);
...
test('derivePageDetails: submitted chunk missing a page renders missing, never extracted', () => {
  ...
  const chunks = [chunk({ startPage: 1, endPage: 3, result: resultOf({ 1: 'one', 3: 'three' }) })];
...
test('derivePageDetails: a submitted page with empty text renders missing, never extracted', () => {
  ...
  const chunks = [chunk({ startPage: 1, endPage: 2, result: resultOf({ 1: 'one', 2: '' }) })];
...
test('derivePageDetails: pending / claimed / not-yet-materialized pages render pending', () => {
  const chunks = [chunk({ startPage: 1, endPage: 2, status: 'claimed', result: null })];
...
test('aggregatePagesText: joins pages in page order, corrections win, blank pages dropped', () => {
  ...
  assert.equal(aggregatePagesText(chunks, corrections), 'one\n\nTWO-FIXED\n\nfour');
});
