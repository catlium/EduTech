...
export default function ContentListPage() {
  ...
  if (scope.subjectId)
    chips.push({ label: scopeName('subject') || 'Subject', onClear: () => clearFilter('subject') });
  if (scope.chapterId)
    chips.push({ label: scopeName('chapter') || 'Chapter', onClear: () => clearFilter('chapter') });
  if (scope.topicId)
    chips.push({ label: scopeName('topic') || 'Topic', onClear: () => clearFilter('topic') });
  if (debouncedSearch)
    chips.push({ label: `“${debouncedSearch}”`, onClear: () => clearFilter('q') });
