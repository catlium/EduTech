...
\noindent This chapter describes each implemented feature area, its data
model, workflow, and integration with the authorization model of
...
(Figure~\ref{fig:academic-scope}): teachers and students get a subject set
...
a lease. Figure~\ref{fig:ocr-processing} shows the distributed flow and
Figure~\ref{fig:ocr-state} the job/chunk state machines.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/ocr-processing-sequence-bw.png}
\caption{Distributed OCR material processing (coordinator + OCR workers).}
...
AI generation (Figure~\ref{fig:ai-generation}) is an asynchronous pipeline:
...
\caption{Asynchronous AI generation (job lifecycle).}
