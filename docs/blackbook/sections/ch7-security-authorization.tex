...
\begin{center}
\textit{This chapter details the security and authorization architecture:
cookie-based authentication and session hardening, CSRF protection, the
permission catalogue, the guard chain, academic-scope enforcement, and
...
session management~\cite{owaspsession}. The login and refresh flow is shown in
Figure~\ref{fig:auth-sequence}; the per-request guard chain is shown in
Figure~\ref{fig:auth-request}.
...
\texttt{sid}, loads the referenced \texttt{\seqsplit{auth\_sessions}} row, and checks that
...
decision chain is rendered in Figure~\ref{fig:permission-model} and
Figure~\ref{fig:permission-model-part2};
the academic-scope engine is detailed in Figures~\ref{fig:academic-scope-resolution}
and~\ref{fig:academic-scope-enforcement}. The per-request guard chain is shown
in Figure~\ref{fig:auth-request}.

\begin{sidewaysfigure}[p]
\centering
\includegraphics[width=500pt]{figures/authentication-guardchain-sequence-bw.pdf}
\par\vspace{1.5ex}
\includegraphics[width=500pt]{figures/authentication-authorization-sequence-bw.pdf}
\caption{Tenant-scoped request flow: (left) the access-token, tenant, and roles
guards; (right) permission-guard evaluation and academic-scope enforcement in
the feature service.}
...
\label{fig:permission-model-part2}
\end{figure}
...
database state.}
\label{fig:academic-scope-resolution}
\end{figure}
...
\label{fig:academic-scope-enforcement}
\end{sidewaysfigure}
